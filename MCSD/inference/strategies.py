...
class BatchStrategy(Strategy):
        ...
        for step in range(self.max_draft_len):
            ...
            if self.draft_model_temp == 0:
                if not self.replacement:
                    ...
                    cand_probs = torch.zeros_like(logits)
                    cand_probs.scatter_(dim=1, index=topk_index, src=topk_probs)
                ...
                cand_probs = torch.softmax(logits / self.draft_model_temp, dim=-1)
    ...
    def verify(
        ...
        outputs: BaseModelOutputWithPast = self.LLM.model(
            input_ids=pruned_input_ids,
            use_cache=True,
        ...
        logits = self.LLM.lm_head(hidden_states[:, -self.max_draft_len - 1 :])
